...
loc = 0;
sca = 1;
[LDA.mu, LDA.sig2] = lognorm_mm_trunc_g_and_h(g, h, loc, sca);
LDA.lognorm_sev = true;
LDA.granularity = 20;
LDA.approx_max = 10 * max(BM.cap);
LDA.tilt = 20 / (2 ^ LDA.granularity);

sev_mean = lognorm_uppexp(0, LDA.mu, LDA.sig2);

Miti = struct;
Miti.effect = lognorm_invcdf([0.0; 0.7], LDA.mu, LDA.sig2) * params.scale;
Miti.cost = [0; 0.3];
...
save('exp/exp_lognorm/exp_ln_wo_BM.mat', 'premium_list', 'detail_list', ...
